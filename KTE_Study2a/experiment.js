...
*/
  ...
  //jMeansBallPresent: Math.round(Math.random()),
  jMeansBallPresent: 1,
  whichButtonPressedArray: new Array(numTrials),  // 0 means f, 1 means j. -1 no response
...
// Goes to description slide
  description: function() {
    ...
    if (experiment.jMeansBallPresent==0) {
      $("#presentButtonSpan1").html("f");
      $("#presentButtonSpan2").html("f");
      $("#absentButtonSpan1").html("j");
      $("#absentButtonSpan2").html("j");
    }
       ...
       videoElement.load();
        ...
        var keyPressHandler = function(event) {
          ...
          { // 32 == Spacebar, 81 = q, 80 = p, 66 = b
            ...
            break; // end of q case
            case 74: // j is pressed

              if (reactionStart==2) { // waiting for reactionTime
                experiment.whichButtonPressedArray[numComplete-1] = 1;
                ...
                if (trial.Ball==1) { // ball is present
                  if (experiment.jMeansBallPresent==1) {
                      experiment.accuracyArray[numComplete-1] = 1;
                      $('#ballDetected').html("Correct. Ball is in the scene!");
                  } else {
                      experiment.accuracyArray[numComplete-1] = 0;
                      $('#ballDetected').html("Incorrect. Ball is in the scene!");
                  }


                } else { // ball is absent but j is pressed
                  if (experiment.jMeansBallPresent==0) {
                    experiment.accuracyArray[numComplete-1] = 1;
                    $('#ballDetected').html("Correct. Ball is not in the scene!");
                  } else {
                    experiment.accuracyArray[numComplete-1] = 0;
                    $('#ballDetected').html("Incorrect. Ball is not in the scene!");
                  }
            ...
            case 70: // f is pressed
              ...
              if (reactionStart==2) { // waiting for reactionTime
                experiment.whichButtonPressedArray[numComplete-1] = 0;
                reactionStart=3; // reactionStart=3 --> reaction was recorded.
                experiment.endTime = (new Date()).getTime();
                experiment.reactionTimeArray[numComplete-1] = experiment.endTime - experiment.startTime;
                if (trial.Ball==1) { // ball is present
                ...
                } else { // ball is absent but f is pressed
            ...
            case "reactionTimeout": // reactionTimeout
                ...
                //if (trial.Ball==1) { // ball is present and timeout
                  ...
                  experiment.accuracyArray[numComplete-1] = 0;
                  $('#ballDetected').html("Too slow in providing response!");
